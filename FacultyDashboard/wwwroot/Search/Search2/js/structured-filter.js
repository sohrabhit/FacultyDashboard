...
$.widget( 'evol.structFilter', {
	...
	_create: function(){
		var bLabels=this.options.buttonLabels,
			...
			h ='<div class="evo-searchFilters" style="float:right"></div>'+
				fnLink('evo-bNew', i18n.bNewCond);
		if(this.options.submitButton){
			h+=fnLink('evo-bSubmit', i18n.bSubmit);
		}
		h +='<div class="evo-editFilter" style="float:right"></div>'+
		...
		if(opVal!==''){
			if(fType!=fTypes.list && (opVal==evoAPI.sIsNull || opVal==evoAPI.sIsNotNull)){
				...
				if(this._step<3){
					...
					switch (fType){
						...
						case fTypes.listDropdown:
							...
							break;
						case fTypes.date:
							//h += '<input id="value" type="text" class="th_date">';
							//break;
						case fTypes.time:
						case fTypes.number:
					...
					if (fType == fTypes.date) {
						...
						editor.find('#value,#value2').pDatepicker({
							initialValue: false,
							format: 'MM/DD/YYYY',//'YYYY/MM/DD',// 'YYYY/MM/DD hh:mm:ss',
							autoClose: true,
							//timePicker: {
							//	enabled: true,
							//	meridiem: {
				...
				if(v){
					...
					addOK = (fType == fTypes.list || fType == fTypes.listDropdown || fType == fTypes.bool || fType == fTypes.date);
				}
			}
