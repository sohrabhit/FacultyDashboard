...
namespace FacultyDashboard.Areas.Resource.Controllers
{
    ...
    public class Resource_6Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResource_6_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Resource_6> _datarepository;
        private readonly ILogger<Resource_6Controller> logger;
        private readonly IMapper _mapper;

        public Resource_6Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Resource_6> _Repository, IResource_6_Repository ar, UserManager<User> userManager
            , ILogger<Resource_6Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Resource_6_ListViewModel Model = new Resource_6_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Resource_6_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Resource_6());
                }
            }
            else
        ...
        {
            Resource_6_Dto Model = new Resource_6_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Resource_6_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Resource_6_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Resource_6");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            myData1 = thedata.Where(x => x.ScienseCount.HasValue).Sum(x => x.ScienseCount);//.Value;//.GroupBy(x => new { start = x..Value },
        ...
        public static Expression<Func<Resource_6, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
