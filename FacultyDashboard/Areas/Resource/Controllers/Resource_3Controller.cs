...
namespace FacultyDashboard.Areas.Resource.Controllers
{
    ...
    public class Resource_3Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResource_3_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Resource_3> _datarepository;
        private readonly ILogger<Resource_3Controller> logger;
        private readonly IMapper _mapper;

        public Resource_3Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Resource_3> _Repository, IResource_3_Repository ar, UserManager<User> userManager
            , ILogger<Resource_3Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Resource_3_ListViewModel Model = new Resource_3_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Resource_3_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Resource_3());
                }
            }
            else
        ...
        {
            Resource_3_Dto Model = new Resource_3_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Resource_3_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Resource_3_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Resource_3");
        ...
        public static Expression<Func<Resource_3, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
