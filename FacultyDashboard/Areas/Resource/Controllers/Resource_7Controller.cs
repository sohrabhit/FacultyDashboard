...
namespace FacultyDashboard.Areas.Resource.Controllers
{
    ...
    public class Resource_7Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResource_7_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Resource_7> _datarepository;
        private readonly ILogger<Resource_7Controller> logger;
        private readonly IMapper _mapper;
        private readonly ISettingRepository _Settingrepository;
        private readonly IRepository<Setting> _Settingdatarepository;

        public Resource_7Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Resource_7> _Repository, IResource_7_Repository ar, UserManager<User> userManager
            , ILogger<Resource_7Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Resource_7_ListViewModel Model = new Resource_7_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Resource_7_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Resource_7());
                }
            }
            else
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            var set = await _Settingrepository.IncreaseResource_7(pat.ScienseCount, pat.SienceGroup, false);
            await _Settingdatarepository.UpdateAsync(set, new CancellationToken());
            await dataHub.Clients.All.SendAsync("GetNotification", "SettingResource_7");
        ...
        public async Task<IActionResult> ExportToExcel(int pageId = 1)
        {
            ...
            var package = await _repository.ExportAllToExcel(10, "دانشجویان مشروط(معدل زیر 12)", Entities.LanguageType.FA, pageId);
        ...
        {
            Resource_7_Dto Model = new Resource_7_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Resource_7_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Resource_7_Dto>(datas);
            if (model != null)
            {
                int setval = 0;
                if (uriidForEdit == 0)
                {
                    ...
                    var set1 = await _Settingrepository.IncreaseResource_7(model.ScienseCount, model.SienceGroup, true);
                    await _Settingdatarepository.UpdateAsync(set1, new CancellationToken());
                }
                else
                {
                    var editmodel = await _datarepository.GetByIdAsync(new CancellationToken(), uriidForEdit);
                    if (editmodel != null && editmodel.ScienseCount.HasValue)
                    {
                        if (editmodel.ScienseCount.Value > model.ScienseCount.Value)
                        {
                            setval = editmodel.ScienseCount.Value - model.ScienseCount.Value;
                            var set1 = await _Settingrepository.IncreaseResource_7(setval, model.SienceGroup, false);
                            await _Settingdatarepository.UpdateAsync(set1, new CancellationToken());
                        }
                        else if (editmodel.ScienseCount.Value < model.ScienseCount.Value)
                        {
                            setval = model.ScienseCount.Value - editmodel.ScienseCount.Value;
                            var set1 = await _Settingrepository.IncreaseResource_7(setval, model.SienceGroup, true);
                            await _Settingdatarepository.UpdateAsync(set1, new CancellationToken());
                        }
                    }
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Resource_7");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            Type thetyp = typeof(SienceGroupType);
            var myData2 = _datarepository.Table.OrderBy(x => x.RegisterDate)
                  .Where(Condition(StartDate, EndDate))
                       .GroupBy(x => new { degree = x.SienceGroup.Value },
                       (y, c) => new ChartWith3ColString()
                       {
                           Col1 = y.degree.GetDisplayName(),
                           Col2 = (c.Where(p => p.Gender.Value == GenderType.Male).Sum(k => k.ScienseCount)).ToString(),//y.sex.GetDisplayName(),//.Sum(k => k.PatientCount).Value.ToString(),
                           Col3 = (c.Where(p => p.Gender.Value == GenderType.Female).Sum(k => k.ScienseCount)).ToString(),//y.sex.GetDisplayName(),//.Sum(k => k.PatientCount).Value.ToString(),
                       }).ToList();
        ...
        public static Expression<Func<Resource_7, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
