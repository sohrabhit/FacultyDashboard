...
namespace FacultyDashboard.Areas.Resource.Controllers
{
    ...
    public class Resource_4Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResource_4_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Resource_4> _datarepository;
        private readonly ILogger<Resource_4Controller> logger;
        private readonly IMapper _mapper;

        public Resource_4Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Resource_4> _Repository, IResource_4_Repository ar, UserManager<User> userManager
            , ILogger<Resource_4Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Resource_4_ListViewModel Model = new Resource_4_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Resource_4_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Resource_4());
                }
            }
            else
        ...
        {
            Resource_4_Dto Model = new Resource_4_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Resource_4_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Resource_4_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Resource_4");
        ...
        public static Expression<Func<Resource_4, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
