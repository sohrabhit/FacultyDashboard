...
namespace FacultyDashboard.Areas.Resource.Controllers
{
    ...
    public class Resource_5Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResource_5_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Resource_5> _datarepository;
        private readonly ILogger<Resource_5Controller> logger;
        private readonly IMapper _mapper;

        public Resource_5Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Resource_5> _Repository, IResource_5_Repository ar, UserManager<User> userManager
            , ILogger<Resource_5Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Resource_5_ListViewModel Model = new Resource_5_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Resource_5_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Resource_5());
                }
            }
            else
        ...
        {
            Resource_5_Dto Model = new Resource_5_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Resource_5_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Resource_5_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Resource_5");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            int? myData1 = 0, myData2 = 0;
            myData1 = thedata.Where(x => x.StudentCount.HasValue).Sum(x => x.StudentCount);//.Value;//.GroupBy(x => new { start = x..Value },
            myData2 = thedata.Where(x => x.MasterCount.HasValue).Sum(x => x.MasterCount);//.Value;
            //var res = myData2 / myData1;
            
            return new JsonResult(new { koldata_1 = myData1, koldata_2 = myData2, doctor = Doctor_id });
        }
        public static Expression<Func<Resource_5, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
