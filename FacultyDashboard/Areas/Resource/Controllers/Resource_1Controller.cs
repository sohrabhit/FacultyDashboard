...
namespace FacultyDashboard.Areas.Resource.Controllers
{
    ...
    public class Resource_1Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResource_1_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Resource_1> _datarepository;
        private readonly ILogger<Resource_1Controller> logger;
        private readonly IMapper _mapper;

        public Resource_1Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Resource_1> _Repository, IResource_1_Repository ar, UserManager<User> userManager
            , ILogger<Resource_1Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Resource_1_ListViewModel Model = new Resource_1_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Resource_1_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Resource_1());
                }
            }
            else
        ...
        {
            Resource_1_Dto Model = new Resource_1_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Resource_1_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Resource_1_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Resource_1");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            ///////
            Type thetyp = typeof(FacultyType);
            var myData2 = thedata.GroupBy(x => new { start = x.Faculty.Value },
        ...
        public static Expression<Func<Resource_1, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
