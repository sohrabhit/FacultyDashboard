...
namespace FacultyDashboard.Areas.Education.Controllers
{
    ...
    public class Education_3Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IEducation_3_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Education_3> _datarepository;
        private readonly ILogger<Education_3Controller> logger;
        private readonly IMapper _mapper;

        public Education_3Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Education_3> _Repository, IEducation_3_Repository ar, UserManager<User> userManager
            , ILogger<Education_3Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Education_3_ListViewModel Model = new Education_3_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Education_3_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Education_3());
                }
            }
            else
        ...
        {
            Education_3_Dto Model = new Education_3_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Education_3_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Education_3_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Education_3");
        ...
        public static Expression<Func<Education_3, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
