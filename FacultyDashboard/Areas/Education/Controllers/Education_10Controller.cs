...
namespace FacultyDashboard.Areas.Education.Controllers
{
    ...
    public class Education_10Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IEducation_10_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Education_10> _datarepository;
        private readonly ILogger<Education_10Controller> logger;
        private readonly IMapper _mapper;

        public Education_10Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Education_10> _Repository, IEducation_10_Repository ar, UserManager<User> userManager
            , ILogger<Education_10Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Education_10_ListViewModel Model = new Education_10_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Education_10_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Education_10());
                }
            }
            else
        ...
        public async Task<IActionResult> ExportToExcel(int pageId = 1)
        {
            ...
            var package = await _repository.ExportAllToExcel(10, "ميانگين معدل سالانه دانشجويان", Entities.LanguageType.FA, pageId);
        ...
        {
            Education_10_Dto Model = new Education_10_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Education_10_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Education_10_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Education_10");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            ///////
            Type thetyp = typeof(AvgEducationType);
            var myData2 = thedata.GroupBy(x => new { start = x.AvgEducationDegree.Value },
                       (y, c) => new ChartWith3ColString()
                       {
                           Col1 = y.start.GetDisplayName(),
                           Col2 = (c.Sum(k => k.Avg).Value / (c.Sum(k => k.Count).Value)).ToString(),
                       }).ToList();
        ...
        public static Expression<Func<Education_10, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
