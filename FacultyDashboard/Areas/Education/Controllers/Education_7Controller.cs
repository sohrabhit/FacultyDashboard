...
namespace FacultyDashboard.Areas.Education.Controllers
{
    ...
    public class Education_7Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IEducation_7_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Education_7> _datarepository;
        private readonly ILogger<Education_7Controller> logger;
        private readonly IMapper _mapper;

        public Education_7Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Education_7> _Repository, IEducation_7_Repository ar, UserManager<User> userManager
            , ILogger<Education_7Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Education_7_ListViewModel Model = new Education_7_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Education_7_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Education_7());
                }
            }
            else
        ...
        {
            Education_7_Dto Model = new Education_7_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Education_7_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Education_7_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Education_7");
        ...
        public static Expression<Func<Education_7, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
