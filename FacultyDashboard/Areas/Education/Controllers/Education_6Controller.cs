...
namespace FacultyDashboard.Areas.Education.Controllers
{
    ...
    public class Education_6Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IEducation_6_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Education_6> _datarepository;
        private readonly ILogger<Education_6Controller> logger;
        private readonly IMapper _mapper;

        public Education_6Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Education_6> _Repository, IEducation_6_Repository ar, UserManager<User> userManager
            , ILogger<Education_6Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Education_6_ListViewModel Model = new Education_6_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Education_6_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Education_6());
                }
            }
            else
        ...
        {
            Education_6_Dto Model = new Education_6_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Education_6_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Education_6_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Education_6");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            ///////
            Type thetyp = typeof(MasterGroupType);
            var myData2 = thedata.GroupBy(x => new { start = x.MasterGroup.Value },
                       (y, c) => new ChartWith3ColString()
                       {
                           Col1 = y.start.GetDisplayName(),
                           Col2 = ((c.Sum(k => k.StudentCount).Value / (c.Sum(k => k.MasterCount).Value))* 100).ToString(),
                       }).ToList();
        ...
        public static Expression<Func<Education_6, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
