...
namespace FacultyDashboard.Areas.Education.Controllers
{
    ...
    public class Education_8Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IEducation_8_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Education_8> _datarepository;
        private readonly ILogger<Education_8Controller> logger;
        private readonly IMapper _mapper;

        public Education_8Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Education_8> _Repository, IEducation_8_Repository ar, UserManager<User> userManager
            , ILogger<Education_8Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Education_8_ListViewModel Model = new Education_8_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Education_8_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Education_8());
                }
            }
            else
        ...
        {
            Education_8_Dto Model = new Education_8_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Education_8_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Education_8_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Education_8");
        ...
        public static Expression<Func<Education_8, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
