...
namespace FacultyDashboard.Areas.Education.Controllers
{
    ...
    public class Education_11Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IEducation_11_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Education_11> _datarepository;
        private readonly ILogger<Education_11Controller> logger;
        private readonly IMapper _mapper;

        public Education_11Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Education_11> _Repository, IEducation_11_Repository ar, UserManager<User> userManager
            , ILogger<Education_11Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Education_11_ListViewModel Model = new Education_11_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Education_11_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Education_11());
                }
            }
            else
        ...
        public async Task<IActionResult> ExportToExcel(int pageId = 1)
        {
            ...
            var package = await _repository.ExportAllToExcel(10, "تعداد دانشجویان المپیاد", Entities.LanguageType.FA, pageId);
        ...
        {
            Education_11_Dto Model = new Education_11_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Education_11_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Education_11_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Education_11");
        ...
        public static Expression<Func<Education_11, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
