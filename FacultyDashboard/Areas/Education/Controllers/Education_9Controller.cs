...
namespace FacultyDashboard.Areas.Education.Controllers
{
    ...
    public class Education_9Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IEducation_9_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Education_9> _datarepository;
        private readonly ILogger<Education_9Controller> logger;
        private readonly IMapper _mapper;

        public Education_9Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Education_9> _Repository, IEducation_9_Repository ar, UserManager<User> userManager
            , ILogger<Education_9Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Education_9_ListViewModel Model = new Education_9_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Education_9_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Education_9());
                }
            }
            else
        ...
        public async Task<IActionResult> ExportToExcel(int pageId = 1)
        {
            ...
            var package = await _repository.ExportAllToExcel(10, "آمار دانشجویان مهمان از سایر دانشکده های دانشگاه ع پ ارتش به دانشکده پیراپزشکی", Entities.LanguageType.FA, pageId);
        ...
        {
            Education_9_Dto Model = new Education_9_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Education_9_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Education_9_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Education_9");
        ...
        public static Expression<Func<Education_9, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
