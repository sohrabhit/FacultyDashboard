...
namespace FacultyDashboard.Areas.Education.Controllers
{
    ...
    public class Education_5Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IEducation_5_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Education_5> _datarepository;
        private readonly ILogger<Education_5Controller> logger;
        private readonly IMapper _mapper;

        public Education_5Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Education_5> _Repository, IEducation_5_Repository ar, UserManager<User> userManager
            , ILogger<Education_5Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Education_5_ListViewModel Model = new Education_5_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Education_5_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Education_5());
                }
            }
            else
        ...
        {
            Education_5_Dto Model = new Education_5_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Education_5_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Education_5_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Education_5");
        ...
        public static Expression<Func<Education_5, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
