...
namespace FacultyDashboard.Areas.Education.Controllers
{
    ...
    public class Education_12Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IEducation_12_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Education_12> _datarepository;
        private readonly ILogger<Education_12Controller> logger;
        private readonly IMapper _mapper;

        public Education_12Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Education_12> _Repository, IEducation_12_Repository ar, UserManager<User> userManager
            , ILogger<Education_12Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Education_12_ListViewModel Model = new Education_12_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Education_12_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Education_12());
                }
            }
            else
        ...
        public async Task<IActionResult> ExportToExcel(int pageId = 1)
        {
            ...
            var package = await _repository.ExportAllToExcel(10, "تعداد دانشجویان استعداد درخشان", Entities.LanguageType.FA, pageId);
        ...
        {
            Education_12_Dto Model = new Education_12_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Education_12_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Education_12_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Education_12");
        ...
        public static Expression<Func<Education_12, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
