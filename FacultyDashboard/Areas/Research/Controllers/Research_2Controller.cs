...
namespace FacultyDashboard.Areas.Research.Controllers
{
    ...
    public class Research_2Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResearch_2_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Research_2> _datarepository;
        private readonly ILogger<Research_2Controller> logger;
        private readonly IMapper _mapper;

        public Research_2Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Research_2> _Repository, IResearch_2_Repository ar, UserManager<User> userManager
            , ILogger<Research_2Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Research_2_ListViewModel Model = new Research_2_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Research_2_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Research_2());
                }
            }
            else
        ...
        {
            Research_2_Dto Model = new Research_2_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Research_2_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Research_2_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Research_2");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            ///////
            Type thetyp = typeof(CeminarType);
            var myData2 = thedata.GroupBy(x => new { start = x.Ceminar.Value },
        ...
        public static Expression<Func<Research_2, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
