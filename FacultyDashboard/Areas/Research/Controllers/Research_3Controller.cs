...
namespace FacultyDashboard.Areas.Research.Controllers
{
    ...
    public class Research_3Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResearch_3_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Research_3> _datarepository;
        private readonly ILogger<Research_3Controller> logger;
        private readonly IMapper _mapper;

        public Research_3Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Research_3> _Repository, IResearch_3_Repository ar, UserManager<User> userManager
            , ILogger<Research_3Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Research_3_ListViewModel Model = new Research_3_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Research_3_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Research_3());
                }
            }
            else
        ...
        {
            Research_3_Dto Model = new Research_3_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Research_3_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Research_3_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Research_3");
        ...
        public static Expression<Func<Research_3, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
