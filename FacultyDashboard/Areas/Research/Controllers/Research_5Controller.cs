...
namespace FacultyDashboard.Areas.Research.Controllers
{
    ...
    public class Research_5Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResearch_5_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Research_5> _datarepository;
        private readonly ILogger<Research_5Controller> logger;
        private readonly IMapper _mapper;

        public Research_5Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Research_5> _Repository, IResearch_5_Repository ar, UserManager<User> userManager
            , ILogger<Research_5Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Research_5_ListViewModel Model = new Research_5_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Research_5_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Research_5());
                }
            }
            else
        ...
        {
            Research_5_Dto Model = new Research_5_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Research_5_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Research_5_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Research_5");
        ...
        public static Expression<Func<Research_5, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
