...
namespace FacultyDashboard.Areas.Research.Controllers
{
    ...
    public class Research_6Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IResearch_6_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Research_6> _datarepository;
        private readonly ILogger<Research_6Controller> logger;
        private readonly IMapper _mapper;

        public Research_6Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Research_6> _Repository, IResearch_6_Repository ar, UserManager<User> userManager
            , ILogger<Research_6Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Research_6_ListViewModel Model = new Research_6_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Research_6_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Research_6());
                }
            }
            else
        ...
        {
            Research_6_Dto Model = new Research_6_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Research_6_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Research_6_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Research_6");
        ...
        public static Expression<Func<Research_6, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
