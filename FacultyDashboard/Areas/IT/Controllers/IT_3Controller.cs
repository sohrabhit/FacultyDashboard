...
namespace FacultyDashboard.Areas.IT.Controllers
{
    ...
    public class IT_3Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IIT_3_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<IT_3> _datarepository;
        private readonly ILogger<IT_3Controller> logger;
        private readonly IMapper _mapper;

        public IT_3Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<IT_3> _Repository, IIT_3_Repository ar, UserManager<User> userManager
            , ILogger<IT_3Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            IT_3_ListViewModel Model = new IT_3_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new IT_3_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new IT_3());
                }
            }
            else
        ...
        public async Task<IActionResult> ExportToExcel(int pageId = 1)
        {
            ...
            var package = await _repository.ExportAllToExcel(10, "آمار دانشجویان بورسیه ( سپاه،ناجا،وزارت دفاع)", Entities.LanguageType.FA, pageId);
        ...
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            ...
            if (model != null)
            {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "IT_3");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            var myData2 = thedata.GroupBy(x => new { start = x.Master.Value },
            ...
            return new JsonResult(new { koldata = final, cate = colname.ToList(), kol = countkol, doctor = Doctor_id });
        }
        public static Expression<Func<IT_3, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
