...
namespace FacultyDashboard.Areas.IT.Controllers
{
    ...
    public class IT_5Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IIT_5_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<IT_5> _datarepository;
        private readonly ILogger<IT_5Controller> logger;
        private readonly IMapper _mapper;

        public IT_5Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<IT_5> _Repository, IIT_5_Repository ar, UserManager<User> userManager
            , ILogger<IT_5Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            IT_5_ListViewModel Model = new IT_5_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new IT_5_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new IT_5());
                }
            }
            else
        ...
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            ...
            if (model != null)
            {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "IT_5");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            var myData2 = thedata//.GroupBy(x => new { start = x.Master.Value },
                       .Select(x => new ChartWith3ColString()
                       {
                           Col1 = x.Title,
                           Col2 = x.Master.Value.GetDisplayName(),
                           Col3 = x.RegisterDate.ToShortDateString()
                       }).ToList();
            
            return new JsonResult(new { koldata = myData2, doctor = Doctor_id });
        }
        public static Expression<Func<IT_5, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
