...
namespace FacultyDashboard.Areas.Student.Controllers
{
    ...
    public class Student_7Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IStudent_7_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Student_7> _datarepository;
        private readonly ILogger<Student_7Controller> logger;
        private readonly IMapper _mapper;

        public Student_7Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Student_7> _Repository, IStudent_7_Repository ar, UserManager<User> userManager
            , ILogger<Student_7Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Student_7_ListViewModel Model = new Student_7_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Student_7_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Student_7());
                }
            }
            else
        ...
        {
            Student_7_Dto Model = new Student_7_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Student_7_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Student_7_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Student_7");
        ...
        public static Expression<Func<Student_7, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
