...
namespace FacultyDashboard.Areas.Student.Controllers
{
    ...
    public class Student_3Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IStudent_3_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Student_3> _datarepository;
        private readonly ILogger<Student_3Controller> logger;
        private readonly IMapper _mapper;

        public Student_3Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Student_3> _Repository, IStudent_3_Repository ar, UserManager<User> userManager
            , ILogger<Student_3Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Student_3_ListViewModel Model = new Student_3_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Student_3_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Student_3());
                }
            }
            else
        ...
        {
            Student_3_Dto Model = new Student_3_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Student_3_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Student_3_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Student_3");
        ...
        public static Expression<Func<Student_3, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
