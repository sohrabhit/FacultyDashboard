...
namespace FacultyDashboard.Areas.Student.Controllers
{
    ...
    public class Student_8Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IStudent_8_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Student_8> _datarepository;
        private readonly ILogger<Student_8Controller> logger;
        private readonly IMapper _mapper;

        public Student_8Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Student_8> _Repository, IStudent_8_Repository ar, UserManager<User> userManager
            , ILogger<Student_8Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Student_8_ListViewModel Model = new Student_8_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Student_8_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Student_8());
                }
            }
            else
        ...
        public async Task<IActionResult> ExportToExcel(int pageId = 1)
        {
            ...
            var package = await _repository.ExportAllToExcel(10, "دانشجویان مهمان از دانشکده پیراپزشکی به سایر دانشکده ها", Entities.LanguageType.FA, pageId);
        ...
        {
            Student_8_Dto Model = new Student_8_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Student_8_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Student_8_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Student_8");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            int countkol = 0;
            if (thedata != null)
                countkol = thedata.Sum(x => x.Count).Value;
            return new JsonResult(new { koldata = countkol, doctor = Doctor_id });
        }
        public static Expression<Func<Student_8, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
