...
namespace FacultyDashboard.Areas.Student.Controllers
{
    ...
    public class Student_2Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IStudent_2_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Student_2> _datarepository;
        private readonly ILogger<Student_2Controller> logger;
        private readonly IMapper _mapper;

        public Student_2Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Student_2> _Repository, IStudent_2_Repository ar, UserManager<User> userManager
            , ILogger<Student_2Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Student_2_ListViewModel Model = new Student_2_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Student_2_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Student_2());
                }
            }
            else
        ...
        {
            Student_2_Dto Model = new Student_2_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Student_2_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Student_2_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Student_2");
        ...
        public async Task<IActionResult> Report(int Doctor_id, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            ///////
            Type thetyp = typeof(CeremonyType);
            var myData2 = thedata.GroupBy(x => new { start = x.Ceremony.Value },
        ...
        public static Expression<Func<Student_2, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
