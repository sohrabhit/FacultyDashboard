...
namespace FacultyDashboard.Areas.Student.Controllers
{
    ...
    public class Student_6Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IStudent_6_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Student_6> _datarepository;
        private readonly ILogger<Student_6Controller> logger;
        private readonly IMapper _mapper;

        public Student_6Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Student_6> _Repository, IStudent_6_Repository ar, UserManager<User> userManager
            , ILogger<Student_6Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Student_6_ListViewModel Model = new Student_6_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Student_6_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Student_6());
                }
            }
            else
        ...
        {
            Student_6_Dto Model = new Student_6_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Student_6_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Student_6_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Student_6");
        ...
        public static Expression<Func<Student_6, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
