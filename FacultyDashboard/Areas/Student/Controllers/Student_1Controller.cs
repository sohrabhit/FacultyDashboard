...
namespace FacultyDashboard.Areas.Student.Controllers
{
    ...
    public class Student_1Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IStudent_1_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Student_1> _datarepository;
        private readonly ILogger<Student_1Controller> logger;
        private readonly IMapper _mapper;

        public Student_1Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Student_1> _Repository, IStudent_1_Repository ar, UserManager<User> userManager
            , ILogger<Student_1Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Student_1_ListViewModel Model = new Student_1_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Student_1_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Student_1());
                }
            }
            else
        ...
        {
            Student_1_Dto Model = new Student_1_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Student_1_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Student_1_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Student_1");
        ...
        public static Expression<Func<Student_1, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
