...
namespace FacultyDashboard.Areas.Student.Controllers
{
    ...
    public class Student_4Controller : Controller
    {
        private IWebHostEnvironment Environment;
        private readonly IRepository<User> _Userrepository;
        private readonly IHubContext<dataHub> dataHub;
        private readonly UserManager<User> userManager;
        private readonly IStudent_4_Repository _repository;
        private readonly IRepository<Notification> _Noti_repository;
        private readonly IRepository<Student_4> _datarepository;
        private readonly ILogger<Student_4Controller> logger;
        private readonly IMapper _mapper;

        public Student_4Controller(IHubContext<dataHub> _DataHub, IRepository<Notification> noti,
            IRepository<Student_4> _Repository, IStudent_4_Repository ar, UserManager<User> userManager
            , ILogger<Student_4Controller> logger, IMapper mapper, IWebHostEnvironment _environment
        ...
        {
            Student_4_ListViewModel Model = new Student_4_ListViewModel();
            if (HttpContext.Request.QueryString.HasValue && HttpContext.Request.QueryString.Value.Contains("&"))
            {
                ...
                {
                    Model = new Student_4_ListViewModel();
                    Model.Datas = _datarepository.Search(SearchParameters, new Student_4());
                }
            }
            else
        ...
        {
            Student_4_Dto Model = new Student_4_Dto();
            if (id.HasValue)
            {
                var model = await _datarepository.GetByIdAsync(new CancellationToken(), id.Value);
                Model = Student_4_Dto.FromEntity(_mapper, model); // Entity => DTO
            }
            return View(Model);
        }
        public async Task<IActionResult> Save(int uriidForEdit, string datas)
        {
            var model = JsonConvert.DeserializeObject<Student_4_Dto>(datas);
            if (model != null)
            {
                if (uriidForEdit == 0)
                {
                ...
                await dataHub.Clients.All.SendAsync("GetNotification", "Student_4");
        ...
        public static Expression<Func<Student_4, bool>> Condition(DateTime? StartDate, DateTime? EndDate)
        {
